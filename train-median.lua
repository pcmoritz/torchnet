...
local opt = lapp[[
   ...
   -b,--batchSize     (default 500)          batch size
   -m,--momentum      (default 0)           momentum, for SGD only
   --coefL1           (default 0)           L1 penalty on the weights
   --coefL2           (default 0)           L2 penalty on the weights
   -t,--threads       (default 32)           number of threads
]]
...
if opt.network == '' then
   ...
   model:add(nn.Tanh())
   model:add(nn.SpatialMedianPooling(2, 2, 2, 2))
...
function train(dataset)
   ...
   for t = 1,dataset:size(),opt.batchSize do
	 ...
	 local result = model:backward(inputs, df_do)
